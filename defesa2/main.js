...
function criarProduto(produto) {
  ...
  rating.textContent = `Avaliações: ${produto.rating.count}`
...
function ordenarProdutosPorPreco(ordem) {
  ...
  if (ordem === 'crescente') {
    produtosOrdenados = produtosList.sort((a, b) => a.rating.count - b.rating.count);
  } else if (ordem === 'decrescente') {
    produtosOrdenados = produtosList.sort((a, b) => b.rating.count - a.rating.count);
  } else {
...
function finalizarCompra() {
  ...
  .then((dados) => {
    mensagemFinal.innerHTML = `
      ...
      Referência de pagamento: <strong>${dados.reference}</strong><br>
      <strong>${dados.message}</strong>
    `;
